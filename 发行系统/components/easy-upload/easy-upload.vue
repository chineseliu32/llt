<template>
	<view>
		<view class="upload">
			<block >
				<view class="uplode-file" v-for="(upload,index) in uploads" :key="index">
		...
		<!-- <button type="primary" v-if="types == 'image'" @tap="upload">确认图片</button> -->
	</view>
</template>
								...
								if(this.type=="icon"){
									if(res.tempFiles[0].size<200000){
										...
										var uper = uni.uploadFile({
											...
											formData: {},
											success(res1) {
												console.log("服务器图片地址",res1)
												// 显示上传信息
												that.$emit('successImage',res1)
											}
										})
									}else {
										uni.showToast({
