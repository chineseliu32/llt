<template>
	<view class="uni-rate">
		<view :key="index" :style="{ marginLeft: margin + 'px' }" @click="_onClick(index)" class="uni-rate__icon" v-for="(star, index) in stars">
			...
			<!-- #ifdef APP-NVUE -->
			<view :style="{ width: star.activeWitch.replace('%','')*size/100+'px',overflow:hidden}" class="uni-rate__icon-on">
				<uniIcons style="text-align: left;" :color="activeColor" :size="size" type="star-filled" />
			</view>
			<!-- #endif -->
			<!-- #ifndef APP-NVUE -->
			<!-- <view :style="{ width: star.activeWitch,top:-size/2+'px' }" class="uni-rate__icon-on"> -->
			<view :style="{ width: star.activeWitch, }" class="uni-rate__icon-on">
				<uniIcons1 :color="activeColor" :size="size" type="star-filled" />
			</view>
			<!-- #endif -->
		</view>
	</view>
...
<script>
	import uniIcons from "../uni-icons/uni-icons2.vue";
	import uniIcons1 from "../uni-icons/uni-icons3.vue";
	export default {
		name: "UniRate",
		components: {
			uniIcons,
