			...
			<view class="none" v-for="(item,index) in indexlist" :key="index" @click="xiang(item.id)">
				<view class="cone">
					...
					<view class="right">
						...
						<view class="cnm">
							<text class="one"
								style="font-size: smaller; color: red;">单价:{{item.sell_price | formatNumber}}元</text>

						</view>
						<view class="cnm">
							<text class="one"
								style="font-size: smaller; ">发售时间:{{item.create_time | time}}</text>
						
						</view>

					</view>
					<view   @click="stopSell(item.id)">
						<image style="  width: 50upx;height: 50upx;margin-right: 50upx;" src="../../static/image/minues.png"
							mode="" ></image>
	...
	export default {
		...
		methods: {
			...
			async getinfo() {
				...
				let res = await this.get(getMyPageListApi, data);
				if (res.code == 0) {
			...
			confirmDel(){
				...
				this.post(removePublishProject, data).then(res => {
					console.log(res);
					if (res.code == 200) {
						this.$api.msg("停止发售成功。");
						this.$router.go(0)
