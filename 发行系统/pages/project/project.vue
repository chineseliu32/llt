					...
					<view class="scroll-view">
						<scroll-view class="scroll-view-box" scroll-y="true">
							...
							<view class="one" style="display: none;">
								...
								<view class="h-box" :class="{'height':height}">
									<view class="hangye" v-for="(item,index) in hangyelist" :key="index">
										<view class="h-title"><text class="marl"
												@click="xzhyid(item.id,item.text)">选择</text> {{item.text}} <text
												class="marr" @click="hyidshow(item.id)">展开</text></view>
										<view class="hid-box" :class="{'show':hid==item.id}">
											<view v-for="(it,i) in item.ch" :key="i" class="hcon"><text class="marl"
													@click="xzhyid(it.id,it.text)">选择</text>{{it.text}}</view>
										</view>
									</view>
								</view>
							</view>
							<view class="one">
								...
								<view class="one2">
									<view class="one22" :class="{'back':biaoqianflag==item.id}"
										v-for="(item,index) in biaoqianlist" :key="index" @click="biaoqin(item)">
										{{item.name}}
									</view>
								</view>
							</view>
							<view class="one">
								...
								<view class="one2">
									...
									<view class="one22" :class="{'back':priceflag=='1001'}"
										@click="price('1001','3000')">
										1001-3000
									</view>
									<view class="one22" :class="{'back':priceflag=='3001'}"
										@click="price('3001','5000')">
										3001-5000
									</view>
									<view class="one22" :class="{'back':priceflag=='5001'}"
										@click="price('5001','7000')">
										5001-7000
									</view>
									<view class="one22" :class="{'back':priceflag=='7001'}"
										@click="price('7001','10000')">
										7001-10000
									</view>
							...
							<view class="one">
								...
								<view class="one2">
									<view class="one22" :class="{'back':amountflag=='0'}" @click="amount('0','1000')">
										0-1000
									</view>
									<view class="one22" :class="{'back':amountflag=='1001'}"
										@click="amount('1001','3000')">
										1001-3000
									</view>
									<view class="one22" :class="{'back':amountflag=='3001'}"
										@click="amount('3001','5000')">
										3001-5000
									</view>
									<view class="one22" :class="{'back':amountflag=='5001'}"
										@click="amount('5001','7000')">
										5001-7000
									</view>
									<view class="one22" :class="{'back':amountflag=='7001'}"
										@click="amount('7001','10000')">
										7001-10000
									</view>
									<view class="one22" :class="{'back':amountflag=='10000'}" @click="amount('10000','')">
										10000以上
									</view>
								</view>
							</view>
							<view class="one" style="display: none;">
								...
								<view class="city-box">
									<view v-for="(item,index) in city" class="typebox" :class="{'typeshow':type0show}">
										<view class="type0"><text @click="xzcityid(item)"
												class="marl">选择</text>{{item.text}}<text class="marr"
												@click="guoshow(item.id)">展开</text></view>
			...
			<view class="none" v-for="(item,index) in indexlist" :key="index" @click="xiang(item.id)">
				<view class="cone">
					...
					<view class="right">
						...
						<text class="one" style="font-size: smaller;">发售:{{item.financing_limit}}份</text>
						<view class="cnm">
							<text class="one" style="font-size: smaller; color: red;">单价:{{item.financing_limit_rmb | formatNumber}}元</text>
	...
	export default {
		...
		data() {
			return {
				...
				labelList: [],
				leixinlist: [], //类型
				biaoqianlist: [{id:1,name:"全部"},{id:2,name: "待申购"},{id:3,name: "申购中"} , {id:4,name: "停止申购"}], //标签
		...
		methods: {
			...
			xiang(type) {
				if (!uni.getStorageSync("userNum")) {
					...
					if (uni.getStorageSync("userNum").memberFeeIsOpen == 0) {
						uni.navigateTo({
							url: '../../pagesA/details/details?id=' + type
						})
						...
						if (uni.getStorageSync("userNum").isFeeMember == "" || uni.getStorageSync("userNum").isFeeMember ==
							...
							setTimeout(() => {
								uni.navigateTo({
									url: "../../pagesA/my/pay?type=" + uni.getStorageSync("userNum")
										.userType
								})
							}, 1500)
						} else {
							uni.navigateTo({
								url: '../details/details?id=' + type
							})
						}
					}
				}
			...
			async getinfo() {
				...
				if (this.minamount != "") {
					data.amountStart = this.minamount
				}
				if (this.maxamount != "") {
					data.amountEnd = this.maxamount
				}
				if (this.biaoqianflag != "") {
					data.projectTag = this.biaoqianflag
				}
				if (this.tradeTypeId != "") {
				...
				let res = await this.get(projectFinancingLoangetPageListApi, data);
				if (res.code == 0) {
