<template>
	<view class="ttttt">
		...
		<view class="conter">
			<view class="last" style="display: none;">
			...
			<view style="padding-bottom: 100upx;">
				<view class="none" v-for="(item,index) in indexlist" :key="index" @click="xiang(item.id)">
					<view class="cone">
						<view class="right" style="margin-top: 20upx;margin-left: 50upx;">
							<text class="o1" style="color: darkgray;">{{item.city}}</text>
							<view class="tpp"
								style="color: black;font-size: medium;font-weight: bold;margin-top: 20upx;">
								<view class="one">
									{{item.address}}
								</view>
								<i class="cate-item iconfont iconshouhoufuwu-zidongpingjia"
									hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70"
									@click="gotoDeatil(item.id)"></i>
							</view>
							<view class="tpp" style="margin-bottom: 40upx;">
								<text class="o1" style="color:black; font-size: small;">{{item.recipient}}</text>
								<text class="o2"
									style="color:black; font-size: small; margin-left: 50upx;">{{item.phone}}</text>
								<view class="label" v-if="item.is_default==1" style="margin-left: 20upx;">
									默认地址
								</view>
							</view>
		...
		<view class="foot">
			...
			<button class="confirm-btn" @click="newaddress()">新建提货地址</button>
	...
	export default {
		...
		methods: {
			...
			async getinfo() {
				...
				let res = await this.get(getUserAddressList, data);
				console.log(res);
				if (res.code == 0) {
					let data = res.result;
					if (data.length < 10) {
