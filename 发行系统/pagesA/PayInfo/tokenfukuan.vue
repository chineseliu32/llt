			...
			<div class="shopdetail">
				...
				<div class="box">
					<view class="box" v-for="(token, index) in tokens" :key="index">
						<dl>
							<dt>
								<image v-if="tokendetal" :src="token.image_url" style="width: 300upx; height: 300upx;"
									mode=""></image>
								<image v-else :src="token.image_url" style="width: 300upx; height: 300upx;" mode="">
								</image>
							</dt>
							<dd>
					...
					<p class="price">
						应付金额:
						<span>¥{{ tokens.map(token => token.financing_limit_rmb).reduce((acc, curr) => acc + curr, 0) | formatNumber }}</span>
					</p>
				</div>
			</div>
	...
	export default {
		...
		onLoad(options) {
			...
			eventChannel.on('acceptDataFromOpenerPage', (data) => {
				this.assetId = data.assetIdtoken;
				this.initData();
				this.getAssetgoubyListProductApi();
			});
		},
		...
		methods: {
				...
				this.$get(`${productTokenListBychanpinProductId}`, {
					...
					if (r.code == 0) {
						...
						for (let i = 0; i < r.result.length; i++) {
							this.tokens.push(r.result[i]); // 将每个元素添加到 tokens 数组
						}
			...
			backward() {
				...
				this.isCountdownRunning = false; // 设置倒计时运行状态为 false
			      ...
			      if (this.countdown > 0) {
			        ...
			        this.post(gettokenization, postData).then(res => {
			          if (res.code == 0) {
			            ...
			            // Exit app (only works in hybrid app built with HBuilder)
			            // plus.runtime.quit();
					...
					success: (res) => {
						if (res.confirm) {
							...
							if (this.payType == 1) {
								...
								url = `../details/details?id=${this.tokens[this.tokens.length - 1].product_id}`
							}
							console.log('用户点击成功');
							uni.navigateBack({
								delta: 2 // 退回到列表外层购买
