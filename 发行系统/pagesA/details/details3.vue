	...
	export default {
		...
		methods: {
			...
			async getinfo(id) {
				...
				console.log(res);
				let data = res.result.professionalFieldTreeList["5"]
				let ress = await this.get(fuwutigongCon, {
					id: id,
				...
				if (ress.code == 0) {
					...
					let arr = ress.result.server_field.split(',');
					data.forEach(item => {
						arr.forEach(it => {
							if (item.id == it) {
