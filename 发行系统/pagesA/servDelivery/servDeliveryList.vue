<template>
	<view class="ttttt">
		...
		<view class="conter">
			...
			<!-- 筛选内容 -->
			<view class="example-body" style="display:none">
					...
					<view class="scroll-view">
						<scroll-view class="scroll-view-box" scroll-y="true">
							...
							<view class="one">
								...
								<lb-picker @confirm="xzhangye" :list="leixinlist" :mode="multiSelector" :level="2"><view>{{fuwuname}}</view></lb-picker>
							</view>
							<!-- <view class="one">
			...
			<view class="none" v-for="(item,index) in indexlist" :key="index"  @click="xiang(item.id)" >
				<view class="cone">
					...
					<view class="right">
						...
						<view class="num">
							<text class="o1">{{item.price_sheet}}万</text>
...
<script>
	...
	import {projectFinancingLoangetPageListApi ,allUserEnum,shaixuan,shailist,hangyeId,deliveryList} from "@/api/user.js";
	import moment from '@/utils/moment';
	export default {
		components: {topSearch,uniRate,  RenDropdownFilter,LbPicker},
		data() {
			return {
		...
		methods: {
			xzhangye(e){
				this.fuwuname=e.item[1].text;
				this.leixinflag=e.item[1].id;
				console.log(e)
			},
			//展开洲
			zhoushow(){
       ...
       // #endif
			...
			async getinfo(){
				...
				if(this.leixinflag!=""){
					data.serverField=this.leixinflag
				}
				if(this.minprice!=""){
				...
				let res=await this.get(deliveryList,data);
				if(res.code==0){
					console.log(res)
			...
			async getShaiXuan(){
				...
				if(res.code==0){
					console.log(res)
					let arr=res.result.professionalFieldTreeList["5"];
					let arr1=[];
					arr.forEach(item=>{
