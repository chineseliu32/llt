					...
					<view class="scroll-view">
						<scroll-view class="scroll-view-box" scroll-y="true">
							...
							<view class="one">
								...
								<view class="one2">
									<view class="one22" :class="{'back':weituoflag==item.id}" 
									v-for="(item,index) in weituolist" :key="index" @click="zxweituo(item)">
										{{item.name}}
									</view>
								</view>
							</view>
			...
			<view class="none" v-for="(item,index) in indexlist" :key="index"  @click="xiang(item.id)" >
				<view class="cone">
					...
					<view class="right">
						...
						<view class="num">
							<text class="o1">{{item.sale_limit}}万</text>
...
<script>
	...
	import {projectFinancingLoangetPageListApi ,allUserEnum,shaixuan,shailist,hangyeId,projectAssetPurList} from "@/api/user.js";
	...
	export default {
        ...
        // #endif
			...
			async getinfo(){
				...
				if(this.weituoflag!=""){
					data.commissionClassification=this.weituoflag
				}
				if(this.minprice!=""){
				...
				let res=await this.get(projectAssetPurList,data);
				if(res.code==0){
			...
			async getShaiXuan(){
				...
				if(res.code==0){
					console.log(res)
					let arr=res.result.professionalFieldTreeList["4"];
					let arr1=[];
					arr.forEach(item=>{
						if(item.pid=="c4"){
							arr1.push(item)
			...
			zxweituo(item){
				console.log(item);
				if(item.id==this.weituoflag){
					this.weituoflag=""
				}else{
					this.weituoflag=item.id
				}
			},
			//点击筛选的类型
			zxleixing(item){
