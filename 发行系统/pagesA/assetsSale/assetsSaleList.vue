...
<!-- 			  <view class="nav-item" :class="{current: filterIndex === 2}" @tap="tabClick(2)">
			  ...
			  <i class="cate-item iconfont iconfenlei1" hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70" @click="showDrawer('showRight')"></i>
					...
					<view class="scroll-view">
						<scroll-view class="scroll-view-box" scroll-y="true">
							...
							<view class="one" style="display: none;">
								...
								<view class="one2">
									<view class="one22" :class="{'back':leixinflag==item.id}" 
									v-for="(item,index) in leixinlist" :key="index" @click="zxleixing(item)">
										{{item.text}}
									</view>
								</view>
							</view>
							<!-- <view class="one">
								<view class="one1" @click="hyShow">
									行业<view class="h-name">
										{{hangye}}
									</view>
								</view>
								
								<view class="h-box" :class="{'height':height}">
									<view class="hangye" v-for="(item,index) in hangyelist" :key="index">
										<view class="h-title" ><text class="marl" @click="xzhyid(item.id,item.text)">选择</text> {{item.text}} <text class="marr" @click="hyidshow(item.id)">展开</text></view>
										<view class="hid-box" :class="{'show':hid==item.id}">
											<view v-for="(it,i) in item.ch" :key="i" class="hcon" ><text class="marl" @click="xzhyid(it.id,it.text)">选择</text>{{it.text}}</view>
										</view>
									</view>
								</view>
							</view> -->
							<view class="one" style="display: ;">
								<view class="one1">
									标签
								</view>
								<view class="one2">
									<view class="one22" :class="{'back':biaoqianflag==item.id}" v-for="(item,index) in biaoqianlist" 
									:key="index" @click="biaoqin(item)">
										{{item.labelName}}
									</view>
								</view>
							</view>
							<view class="one">
								...
								<view class="one2">
									<view class="one22" :class="{'back':priceflag=='0'}" @click="price('0','1000')">
										0-1000
									</view>
									<view class="one22"  :class="{'back':priceflag=='1001'}" @click="price('1001','3000')">
										1001-3000
									</view>
									<view  class="one22" :class="{'back':priceflag=='3001'}" @click="price('3001','5000')">
										3001-5000
									</view>
									<view class="one22" :class="{'back':priceflag=='5001'}" @click="price('5001','7000')">
										5001-7000
									</view>
									<view class="one22" :class="{'back':priceflag=='7001'}" @click="price('7001','10000')">
										7001-10000
									</view>
									<view class="one22" :class="{'back':priceflag=='10000'}" @click="price('10000','')">
										10000以上
									</view>
								</view>
							</view>
							<view class="one" style="display: none;">
								<view class="one1" @click="zhoushow">
									地点<view class="h-name">
									{{cityname}}
									</view>
								</view>
								<view class="city-box">
							...
							<view class="close">
								<view class="word-btn" hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70"  @click="closeDrawer('showRight')"><text class="word-btn-white">重置</text></view>
								<view class="word-btn" hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70" @click="ok('showRight')"><text class="word-btn-white">确定</text></view>
							</view>
						</scroll-view>
					</view>
				</uni-drawer>
			...
			<view class="none" v-for="(item,index) in indexlist" :key="index"  @click="xiang(item.id)" >
				<view class="cone">
					...
					<view class="right">
						...
						<view class="label" v-for="(item1,index) in item.label" :key="index" >
							{{item1.labelName}}
						</view>
						<text class="one" style="font-size: smaller;">发售:{{item.project_count}}份</text>
						<view class="cnm">
							<text class="one" style="font-size: smaller; color: red;">单价:{{item.sell_price | formatNumber}}元</text>
...
<script>
	...
	import {projectFinancingLoangetPageListApi ,allUserEnum,shaixuan,shailist,hangyeId,saleList,labelList} from "@/api/user.js";
	import moment from '@/utils/moment';
	export default {
		components: {topSearch,uniRate,  RenDropdownFilter},
		data() {
			return {
				current: 0,
				value: '',
		...
		methods: {
				...
				if(!uni.getStorageSync("userNum")) {
					...
					if(uni.getStorageSync("userNum").memberFeeIsOpen==0){
						...
						if(uni.getStorageSync("userNum").isFeeMember==""||uni.getStorageSync("userNum").isFeeMember==0){
							...
							setTimeout(()=>{
								uni.navigateTo({
									url:"../../pages/my/pay?type="+uni.getStorageSync("userNum").userType
								})
							},1500)
						}else{
							uni.navigateTo({
			...
			getProductList() {
				this.$get(`${projectFinancingLoangetPageListApi}`,{projectName : this.keyword}).then(r => {
					// console.log(r)
       ...
       // #endif
			...
			initData2(){
				this.$get(`${projectFinancingLoangetPageListApi}`,{orderType: this.filterParams}).then(r => {
					// console.log(r)
					this.indexlist = r.result.list
					// console.log(this.indexlist)
				}).catch((err) => {
			...
			async getinfo(){
				...
				if(this.leixin!=""){
					data.propertyType=this.leixinflag//资产分类
				}
				if(this.minprice!=""){
				...
				let res=await this.get(saleList,data);
				if(res.code==0){
			...
			async getShaiXuan(){
				...
				if(res.code==0){
					console.log(res)
			/* 		let arr=res.result.professionalFieldTreeList["3"];
					let arr1=[];
					arr.forEach(item=>{
						if(item.pid=="c3"){
							arr1.push(item)
			...
			async getData(id) {
				this.num++;
				let res = await this.get(saleList, {
					projectLabel: id,
					pageNum: this.num
				});
				console.log(res);
			...
			ok(e){
				...
				if(this.filterIndex!=""){
					data.orderType=this.filterIndex
				}
				if(this.keyword!=""){
					data.projectName=this.keyword
				}
