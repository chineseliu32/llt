			...
			<view class="none" v-for="(item,index) in indexlist" :key="index"  @click="xiang(item.id)" >
				<view class="cone">
					...
					<view class="right">
						...
						<view class="num">
							<text class="o1">{{item.purchase_price}}万</text>
...
<script>
	...
	import {projectFinancingLoangetPageListApi ,allUserEnum,shaixuan,shailist,hangyeId,procureList} from "@/api/user.js";
	...
	export default {
        ...
        // #endif
			...
			async getinfo(){
				...
				if(this.leixinflag!=""){
					data.professionalField=this.leixinflag
				}
				if(this.minprice!=""){
				...
				let res=await this.get(procureList,data);
				if(res.code==0){
