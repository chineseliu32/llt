<template>
<uni-shadow-root class="@vant-weapp-skeleton-index"><view v-if="loading" :class="'custom-class '+(utils.bem('skeleton', [{animate}]))">
  ...
  <view :class="utils.bem('skeleton__content')">
    ...
    <view v-for="(item,index) in (rowArray)" :class="'row-class '+(utils.bem('skeleton__row'))" :style="'width:' + (isArray ? rowWidth[index] : rowWidth)"></view>
  </view>
</view>
...
global['__wxRoute'] = '@vant/weapp/skeleton/index'
...
export default global['__wxComponents']['@vant/weapp/skeleton/index']
</script>
