...
VantComponent({
  ...
  methods: {
	...
	checkPermission: async function() {
		...
		if (Array.isArray(this.data.capture)) {
			for (const item of this.data.capture) {
				if (item == 'album') {
					album = true;
				}
				if (item == 'camera') {
		...
		if (typeof(this.data.capture) == 'string') {
			if (this.data.capture == 'album') {
				album = true;
			}
			if (this.data.capture == 'camera') {
				camera = true;
			}
		}
		
		if (album && (status === null || status === 1)) {
			status = permission.isIOS ? await permission.requestIOS('album') :
				await permission.requestAndroid('android.permission.READ_EXTERNAL_STORAGE');
		}
	
		if (camera && (status === null || status === 1)) {
			status = permission.isIOS ? await permission.requestIOS('camera') :
